...
def paste(bot: Bot, update: Update, args: List[str]):
    ...
    if message.reply_to_message:
        ...
        message.reply_text(sbt(chat_id, "What am I supposed to do with this?!"))
        return

    r = requests.post(f'{BASE_URL}/documents', data=data.encode('utf-8'))

    if r.status_code == 404:
        update.effective_message.reply_text(sbt(chat_id, 'Failed to reach dogbin'))
        r.raise_for_status()
...
def get_paste_content(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        message.reply_text(sbt(chat_id, "Please supply a paste key!"))
        return
    ...
    if r.status_code != 200:
        try:
            ...
            if r.status_code == 404:
                ...
                update.effective_message.reply_text(sbt(chat_id, 'Unknown error occured'))
        r.raise_for_status()

    try:
        update.effective_message.reply_text(r.text)
    except BadRequest as excp:
        message.reply_text(sbt(chat_id, 'The paste is too long.'))
        return
